...
bool ZProbe::configure(ConfigReader& cr)
{
    ...
    if(!pin.from_string(cr.get_string(m, probe_pin_key, "nc")) || !pin.as_input() || !pin.connected()) {
        printf("ERROR: config-zprobe: no pin or illegal pin\n");
        return false;
...
bool ZProbe::handle_gcode(GCode& gcode, OutputStream& os)
{
    ...
    if(gcode.get_code() >= 29 && gcode.get_code() <= 32) {
        ...
        if( gcode.get_code() == 30 ) { // simple Z probe
            ...
            probe_result = ((set_z || gcode.has_arg('L')) ? run_probe(mm, rate, -1, reverse) : run_probe_return(mm, rate, -1, reverse));
