...
#define USB_OTG_DMA_SUPPORT         0 // (defined(USB_OTG_HS) && 1)
...
#define USB_TOTAL_FIFO_SIZE(HANDLE) (1024*4) // 4K for STM32H7xx
#endif
...
__weak void USB_vAllocateEPs(USB_HandleType * pxUSB)
{
    ...
    ulFifoOffset = 13           /* JM++ to receive SETUP packets on the control endpoint */
    ...
    if(ulFifoOffset < 0x80) ulFifoOffset= 0x80; // JM++
