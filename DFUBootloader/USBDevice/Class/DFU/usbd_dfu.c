...
#else
                #if 0 // old way erases entire flash
                /* Erase firmware on the first block */
                if (itf->Address == (uint8_t*)DFU_APP(itf)->Firmware.Address) {
                    itf->DevStatus.Status = DFU_APP(itf)->Erase(
                                                itf->Address);
                }
                #else // new way is to erase each sector as needed
                /* will erase an entire sector only once */
                itf->DevStatus.Status = DFU_APP(itf)->Erase(itf->Address);
                #endif
                /* Write after erase */
