...
int cdc_write(uint8_t *ptr, int32_t len)
{
	int retval = -1;
	uint16_t head = console_if_IN.head, tail = console_if_IN.tail;
...
static void console_if_out_cmplt(void* itf, uint8_t * pbuf, uint16_t length)
{
	// we have a buffer from Host, stick it in the rx buffer
	for (uint32_t i = 0; i < length; ++i) {
		RingBufferPut(rx_rb, pbuf[i]);
	}
}

static void console_if_recv(void)
{
	USBD_CDC_Receive(console_if, console_if_OUT.buffer, STDIN_BUFFER_SIZE);
}

int cdc_read(uint8_t *ptr, int32_t len)
{
